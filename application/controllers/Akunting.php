...
class Akunting extends CI_Controller {
	...
	public function get_akun() {
 	...
 	public function tambah_ju() {
		...
		$data = array(
 		...
 		elseif($trans==111) { //Bangunan

 		$data = array(
			'tanggal' 		=>	date('Y-m-d',strtotime($this->input->post('tanggal',TRUE))),
			'nama_perkiraan'=> 	'Bangunan',
			'debet' 		=>	str_replace('.','',$this->input->post('biaya',TRUE)),
			'kredit'		=>	0,
			'keterangan' 	=> 	$this->input->post('keterangan',TRUE)
		);
	...
	}elseif($trans==112) { //Akum.Peny Bangunan
		$data = array(
			'tanggal' 		=>	date('Y-m-d',strtotime($this->input->post('tanggal',TRUE))),
			'nama_perkiraan'=> 	'Akum.Peny Bangunan',
			'debet' 		=>	str_replace('.','',$this->input->post('biaya',TRUE)),
			'kredit'		=>	0,
			'keterangan' 	=> 	$this->input->post('keterangan',TRUE)
		);
		...
		elseif ($trans==11) { // Biaya Penyusutan Kendaraan
		$data = array(
			'tanggal' 		=>	date('Y-m-d',strtotime($this->input->post('tanggal',TRUE))),
